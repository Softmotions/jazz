...
Program.prototype.process = function(namespace, mediator, cb) {
    if (arguments.length === 2) {
        cb = mediator;
        mediator = null;
    }
    if (mediator != null && typeof mediator === "function") {
        this.fn.__jazz_mediator = mediator;
    }
...
Program.prototype._namespaceToArgs = function(namespace) {
    var args = new Array(this.globals.length);
    for (var i = 0; i < this.globals.length; i++) {
        args[i] = namespace[this.globals[i]];
    }
    return args;
};
...
Compiler_JS.prototype._prologue = function(globals) {
    ...
    code += "var __jazz_mediator = this.__jazz_mediator;\n";
    ...
    code += "    __jazz_cb = null;\n";
    code += "    __jazz_output = null;\n";
    code += "    __jazz_emitter = null;\n";
    code += "    __me.__jazz_mediator = __jazz_mediator = null;\n";
    code += "  }\n";
    code += "}\n";

    code += "__jazz_emitter.addListener('data', function(pos, chunk, mediatorOpts, dynamic) {\n";
    code += "  __jazz_output[pos] = __jazz_mediator ? __jazz_mediator(chunk, mediatorOpts, dynamic) : chunk;\n";
    code += "  __jazz_seen++;\n";
...
Compiler_JS.prototype._compileStatement = function(stmt) {
    ...
    else if (stmt.constructor == ast.Define) {
        code += this._compileDefine(stmt);
    }
...
Compiler_JS.prototype._compileEcho = function(stmt) {
    ...
    var cbObj = {dynamic : false};
    code += "__jazz_emitter.emit('data', " + slotVar + ", " + this._compileExpr(stmt.value, cbObj) + ", null, " + cbObj.dynamic + ");\n";
...
Compiler_JS.prototype._compileCall = function(stmt) {
    ...
    code += "function(data, mediatorOpts) { __jazz_emitter.emit('data', " + slotVar + ", data, mediatorOpts, true); });\n";
...
Compiler_JS.prototype._compileDefine = function(stmt) {
    return "var " + stmt.ident.name + " = " + this._compileExpr(stmt.expr) + ";\n";
};
...
Compiler_JS.prototype._compileExpr = function(expr, cbObj) {
    cbObj = cbObj || {};
